...
ComplexNumber ComplexNumber::operator/(const ComplexNumber &c){
	...
	return ComplexNumber((real*c.real+imag*c.imag)/x,(-real*c.imag+imag*c.real)/x);
}

ComplexNumber operator/(double s, const ComplexNumber &c){
    double x = pow(c.real, 2) + pow(c.imag, 2);
	return ComplexNumber(s*c.real/x,-s*c.imag/x);
}
...
double ComplexNumber::angle(){
    if(real < 0 && imag > 0) return 180.0 + atan(imag/real)*180/M_PI;
    if(real < 0 && imag < 0) return -180.0 + atan(imag/real)*180/M_PI;
